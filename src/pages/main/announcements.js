export async function run() {
    ...
    hlp.load(async function () {
        ...
        // TODO: Make this get &path from url if it exists.
        await $.ajax({
            ...
            success: function (communications) {
                ...
                // Get each announcement that is listed from the API
                    ...
                    `).children().off().on("click", async function (event) {
                        ...
                        await $.ajax({
                            ...
                            success: async (comminfo) => {
                                await $.ajax({
                                    url: hlp.api(`/cmd/getannouncement?_token=${hlp.session.token}&packagetype=data&entityid=${hlp.session.domainid}&path=${$(event.target).attr("path")}`),
                                    method: "GET",
                                    dataType: "json",
                                    contentType: "application/json; charset=utf-8",
                                    success: (commdetails) => {
