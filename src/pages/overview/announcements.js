    ...
    await hlp.load(async function () {
        await $("#root").html(`
            ...
            <div class="flex flex-col gap-5 pt-[2rem] mt-[46px] mb-[1.7rem] container mx-auto py-10 px-4">
                <div id="communications" class="flex flex-col gap-5"></div>
            </div>    
            <!---->
            <!---->
        ...
        async function call() {
            ...
            await hlp.prevent_errors(async function () {
                ...
                if (communications.response.code != "OK") {
                    communications = [];
            ...
            if (communications.length != 0) {
                // Sort announcements by closest date
                communications.response.announcements.announcement.sort((a, b) => new Date(b.startdate) - new Date(a.startdate));
    
                if (hlp.page.getparams != undefined && hlp.page.getparams[0].path != undefined) {
                    await content(hlp.page.getparams[0].path);   
                }

                await $.each(communications.response.announcements.announcement, async function (i, communication) {
                    $("#communications").append(`
                        <div path="${communication.path}" class="relative flex flex-row justify-between container mx-auto ${hlp.gettheme("theme-card")} rounded-xl cursor-pointer py-3 px-3">
                            ...
                            ${(()=>{
                                if (hlp.settings.chip_indicators) {
                                    return communication.viewed ? "" : `<div id="unviewed-${communication.path.replace(".zip", "")}" class="absolute pointer-events-none inline-flex right-0 top-0 h-4 w-4 -m-1 animate-ping duration-700 rounded-full ${hlp.gettheme("bg", "700")} opacity-75 justify-center items-center"></div>`;
                                } else {
                                    return "";
                                }
                            })()}
                        </div>
                    `).children().off().click(async function (e) {
                        hlp.load(async function () {
                            await content($(e.target).attr("path"));
                        })
                    })
                })
    
                if ($("#communications div").length == 0) {
                    $("#communications").append(`
                            ...
                            <div class="flex flex-row justify-between container mx-auto cursor-pointer">
                                <div class="flex flex-row justify-center items-center pointer-events-none w-full">
                                    <div class="flex flex-col justify-center items-center">
                                        <h1 class="text-[18px] font-bold">You have no announcements.</h1>
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            async function content(path) {
                ...
                await hlp.prevent_errors(async function () {
                    comminfo = await $.ajax({
                        url: hlp.api(`/cmd/getannouncementinfo?_token=${hlp.session.token}&packagetype=data&entityid=${hlp.session.domainid}&path=${path}`),
                        method: "GET",
                        dataType: "json",
                        contentType: "application/json; charset=utf-8"
                    });
                ...
                if (comminfo.length != 0 && commdetails.length != 0) {
                    await $("#communications").parent().append(`
                        <div id="opened" class="relative flex flex-col justify-between container mx-auto ${hlp.gettheme("theme-card")} rounded-xl py-3 px-3">
