    ...
    await hlp.load(async function () {
        await $("#root").html(`
            ...
            <div class="flex flex-col gap-5 pt-[2rem] mt-[46px] mb-[1.7rem] container mx-auto py-10 px-4">
                <div class="flex flex-col gap-5 container mx-auto ${hlp.gettheme("theme-card")} rounded-xl py-3 px-3">
                    <div class="flex flex-row gap-5">
                        <div class="rounded-full text-white transition ${hlp.gettheme("bg", "600")} ${!hlp.get("pfp").includes("data:") ? "" : `bg-[url('${hlp.get("pfp")}')] bg-cover bg-no-repeat bg-center`} border-[6px] hover:${hlp.gettheme("border", "400")} ${hlp.gettheme("border", "500")} h-20 w-20 flex items-center justify-center text-2xl font-bold uppercase">
                    ...
                    <button id="full-logout" class="w-full px-4 ${hlp.gettheme("theme-shadow")} py-3 bg-red-600 text-white transition font-semibold rounded-xl hover:bg-red-500 focus:outline-none focus:ring-2 focus:ring-red-700 focus:ring-opacity-50">Remove From This Device</button>
                </div>
            </div>
            <!---->
            <!---->
                    ...
                    hlp.load(async function () {
                        await $.ajax({
                            ...
                            success: async function () {
                                ...
                                hlp.set("theme", {"sync":true,"theme":"dark","color":"blue"});
                                hlp.set("settings", [{"option":"include-self","$value":true},{"option":"chip-indicators","$value":true},{"option":"hide-excused","$value":false}]);
                                
                                $("body").removeClass("bg-white").removeClass("bg-black");
                                $("body").removeClass("text-white").removeClass("text-black");

                                $("body").addClass(`${hlp.gettheme("theme-bg")} ${hlp.gettheme("theme-text")}`);
